---plugin/auto-run.lua — :AutoRun user command (Phase 1 surface).
---
---Subcommands: list | show | validate | import | doctor | set-dir.
---Output goes through print()/nvim_echo (user-invoked, not a main
---path); errors are echoed, never vim.notify'd.
...
local SUBCOMMANDS = { "list", "show", "validate", "import", "doctor", "set-dir" }
...
vim.api.nvim_create_user_command("AutoRun", function(cmd)
  ...
  nargs = "*",
  desc = "auto-run: run-config store operations (ADR-0048 Phase 1)",
  complete = function(arglead, cmdline, _)
    -- Complete the subcommand in position 1; config names for `show`.
    ...
    if words[2] == "show" or words[2] == "import" then
